...
"""
    setup(o, nsx, nsy, vxhw, vyhw, [ixhw, iyhw, subgrid, subgrid_gaus,
        itstep, ntrac, score_method, score_th0, score_th1, vxch, vych, dtlimit,
...
- `vych::Union{Real, Nothing}=nothing`: If non-`nothing`, the max tolerant vy
    change between two consecutive tracking.
- `dtlimit::Union{Real, Nothing}=nothing`: If non-`nothing`, time difference of two images is limited.
...
function setup(o::VTT, nsx::Int, nsy::Int; vxhw::Union{Real, Nothing}=nothing, vyhw::Union{Real, Nothing}=nothing,
            ...
            vych::Union{Real, Nothing}=nothing, dtlimit::Union{Real, Nothing}=nothing, peak_inside_th::Union{Real, Nothing}=nothing,
    ...
    dtlimit === nothing && (dtlimit = -999.0) # <=0 for nothing (not to set)
    ...
    set_optional!(o, subgrid, subgrid_gaus, score_method, score_th0, score_th1, peak_inside_th, min_contrast, vxch, vych, dtlimit, use_init_temp, min_visible)
    o.setuped = true
end
...
- `min_visible::Int`: Minimum number of visible values to calculate score when `chk_mask` is true.
"""
function set_optional!(o::VTT, subgrid::Bool, subgrid_gaus::Bool, score_method::String, score_th0::AbstractFloat, score_th1::AbstractFloat, peak_inside_th::Real, min_contrast::Real, vxch::Float64, vych::Float64, dtlimit::Float64, use_init_temp::Bool, min_visible::Int)
    o.subgrid = subgrid
...
function do_tracking(o::VTT, tid0, x0, y0, vx0, vy0, out_subimage::Bool, out_score_ary::Bool)
    ...
    for m = 1:len
        for j = 1:o.ntrac
            ...
            # if o.dtlimit > 0 && abs(dt) > o.dtlimit
            #     status[m] = 11
            #     # @info "(m=$m) Stop tracking at checkpoint 11 (during `dtlimit` check)"
            #     continue
            # end
